...
train_pipeline = [
    ...
    # dict(type='PartOcclusion', parts=(0,), p=0),
    # dict(type='JointJitter', jitter_amount=0.01, p=0.2),

    dict(type='GeneratePoseTarget', with_kp=True, with_limb=False),
    dict(type='FormatShape', input_format='NCTHW_Heatmap'),
    dict(type='Collect', keys=['imgs', 'label'], meta_keys=[]),

    # dict(type='FrameOcclusion', p=0.1),
    # dict(type='BlockOcclusion', height=0.2, p=0.5),
    # dict(type='KeyframeSamples', num_keyframes=16),
...
test_pipeline = [
    ...
    dict(type='PoseCompact', hw_ratio=1., allow_imgpad=True),
    dict(type='Resize', scale=(64, 64), keep_ratio=False),
    dict(type='GeneratePoseTarget', skeletons=skeletons,
